...
router.get('/', async (req, res) => {
  try {
    ...
    const allExams = microsoftCertifications.map((cert: typeof microsoftCertifications[0]) => {
    ...
    // Sort by code
    allExams.sort((a: typeof allExams[0], b: typeof allExams[0]) => a.code.localeCompare(b.code));
...
router.get('/:code', async (req, res) => {
  try {
      ...
      domain.objectives.forEach((objective) => {
        objective.topics.forEach((et) => {
          ...
          if (topic.techTags && topic.techTags.length > 0) {
            topic.techTags.forEach((tag: string) => {
          ...
          if (topic.keyConcepts && Array.isArray(topic.keyConcepts) && topic.keyConcepts.length > 0) {
            topic.keyConcepts.forEach((concept: any) => {
    ...
    const videosByTechTags = Array.from(techTagsMap.entries()).map(([tag, videos]) => ({
      tag,
      videos: Array.from(
        new Map(videos.map((v: any) => [v.video.id, v])).values()
      ),
    }));
