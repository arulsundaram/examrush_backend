...
router.post('/topics/:topicId/prompt-clip', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const scoredTopics = topics.map((t) => {
      ...
      const titleMatches = promptWords.filter((pw) =>
        titleWords.some((tw) => tw.includes(pw) || pw.includes(tw))
      ).length;

      const keywordMatches = promptWords.filter((pw) =>
        keywordWords.some((kw) => kw.includes(pw) || pw.includes(kw))
      ).length;
    ...
    const getConfidence = (score: number, maxScore: number): 'High' | 'Medium' | 'Low' => {
